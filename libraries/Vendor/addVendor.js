...
function refreshList() {
  if (sessionStorage.getItem("email") === null)
  {
          $("#user").html(
              ...
              <button class="btn btn-success my-2 my-sm-2" onclick="login()" type="submit">Login <span class="glyphicon glyphicon-log-in"></span></span></button>
            `
          )
 } else {
          $("#user").html(
              `<button class="btn btn-success my-2 my-sm-2" onclick="logout()" type="submit">Logout <span class="glyphicon glyphicon-log-out"></span></span></button>
           `
          )
      }  
      ...
      for (let ven of data) {
        $('#vendorList').append(
         ...
         <b> ${ven.email} </b>
          ...
          <tr>
          <td>
          <button class="btn btn-danger" id="delVendor" onclick="deleteVendor(${ven.id})"><span class="glyphicon glyphicon-remove"></span>remove</button>
          </td>
          </td>
          </table>
          </div>
